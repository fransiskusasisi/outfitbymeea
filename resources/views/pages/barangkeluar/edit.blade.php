    ...
    <div class="p-8 space-y-6 ">
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
            <div class="w-1/2 mx-auto p-6 overflow-x-auto">
                @if (role() === 'pemilik')
                    <form action="{{ route('pemilik.barangkeluar.update', $barangkeluar->keluar_id) }}" method="POST">
                    @elseif(role() === 'petugas_gudang')
                        <form action="{{ route('gudang.barangkeluar.update', $barangkeluar->keluar_id) }}" method="POST">
                        @elseif(role() === 'kasir')
                            <form action="{{ route('kasir.barangkeluar.update', $barangkeluar->keluar_id) }}"
                                method="POST">
                @endif
                @method('PUT')
                @csrf
